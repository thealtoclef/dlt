...
@pytest.fixture
def normalizer() -> ArrowItemsNormalizer:
    schema = Schema("test_schema")
    config = NormalizeConfiguration()
    # Mock dependencies as we only test the internal _flatten method
    return ArrowItemsNormalizer(None, None, None, schema, "test_load_id", config, collector=NULL_COLLECTOR)

@pytest.fixture
def naming(normalizer: ArrowItemsNormalizer) -> NamingConvention:
    return normalizer.schema.naming

def test_ac1_basic_arrow_flatten(normalizer: ArrowItemsNormalizer) -> None:
    ...
    flattened, _ = normalizer._flatten_py_arrow_item(table, "test_table")
...
def test_ac2_keep_original_not_supported(normalizer: ArrowItemsNormalizer) -> None:
...
def test_ac3_json_string_expansion(normalizer: ArrowItemsNormalizer) -> None:
    ...
    flattened, _ = normalizer._flatten_py_arrow_item(table, "test_table", json_expansion_cols=expansion_cols)
...
def test_ac4_max_depth_equivalent(normalizer: ArrowItemsNormalizer) -> None:
    ...
    flattened, _ = normalizer._flatten_py_arrow_item(table, "test_table", max_nesting=1)
    ...
    flattened, _ = normalizer._flatten_py_arrow_item(table, "test_table", max_nesting=2)
    assert "data__child__val" in flattened.column_names
    assert flattened["data__child__val"].to_pylist() == [42]

def test_ac5_is_nested_type_protection(normalizer: ArrowItemsNormalizer) -> None:
    ...
    # Set up schema to protect the 'protected' column
    ...
    flattened, _ = normalizer._flatten_py_arrow_item(table, "test_table", columns_schema=normalizer.schema.get_table_columns("test_table"))
...
def test_chunked_streaming_parity(normalizer: ArrowItemsNormalizer) -> None:
